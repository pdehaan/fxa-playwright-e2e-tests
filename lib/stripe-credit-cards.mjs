...
export default class StripeCreditCards {
  ...
  static get stripeCards() {
    ...
    ].reduce((map, [brand = "", cards = []]) => {
      map.set(
        brand,
        // Inject the credit card `brand` into each credit card record.
  ...
  /**
   * @param {number} [years=5] Maximum number of years in the future for the credit card expiry.
   * @returns {CardExpiry} An object containing a future `month` and `year` property.
   */
  static getRandomExp(years = 5) {
    const cc_date = faker.date.future(years);
    // NOTE: JavaScript months are zero based.
    const month = cc_date.getMonth() + 1;
    // Convert to 2-digit year (we want "22", not "2022", or "102").
    const year = cc_date.getYear() - 100;
    return {
      month,
      year,
    };
